import { workdaysBetween, getWeekBuckets } from './models.js';
...
export function computeKPIs(){
  ...
  const paceExpected = q.target * (wdSoFar / Math.max(1, wdTotal));
  const delta = done - paceExpected;
  const remaining = Math.max(0, q.target - done);
  const weeks = Math.max(1, Math.ceil((end - todayUTC) / (1000*60*60*24*7)));
  ...
  return { target: q.target, done, pipeline, forecast,
    paceExpected, delta, coverage, runRate, wdTotal, wdSoFar, health, start, end };
}
...
export function weeklySeries(){
  ...
  const start = new Date(q.startDate + "T00:00:00Z");
  const end = new Date(q.endDate + "T00:00:00Z");
  const weeks = getWeekBuckets(start, end);
  const labels = weeks.map((w,i)=>`W${i+1}`);

  // target cumulativo lineare per settimana
  const perWeekTarget = q.target / weeks.length;
  const targetCum = [];
  let sumTarget = 0;
  for (let i=0;i<weeks.length;i++){ sumTarget += perWeekTarget; targetCum.push(Math.round(sumTarget)); }

  // cumulativo fatto per settimana (chiusi "vinto" con expectedCloseDate nella settimana)
...
export function funnelData(){
  ...
  const cfg = getConfig();
  const deals = getDeals(q.id);
  const stageOrder = Object.keys(cfg.stageProbabilities);
  const sums = stageOrder.map(st => deals.filter(d=>d.stage===st).reduce((s,d)=>s+(+d.value||0), 0));
  return { labels: stageOrder, values: sums };
}
